...
struct DIBAN            /*游戏底板结构,表示每个点所具有的属性*/
{
    int var;            /*当前状态 只有0和1,1表示此点已被占用*/
    int color;          /*颜色,游戏底板的每个点可以拥有不同的颜色.增强美观*/
}Table_DIBAN[Vertical_boxs][Horizontal_boxs];
...
/*初始化方块内容.即定义MAX_BOX个FANGKUAI类型的结构数组，并初始化*/
struct FANGKUAI FANGKUAIs[MAX_BOX]=
{
/*
 *   口     口口口   口口      口
 *   口     口         口  口口口
...
{
    int x,y;
    char defen_str[5];/*保存游戏得分*/
    setfillstyle(SOLID_FILL,BgColor);
    x=LeftWin_x;
    y=100;
    bar(x-BSIZE,y,x+BSIZE*3,y+BSIZE*3);
    sprintf(defen_str,"%3d",defen);
    outtextxy(x,y,"defen");
...
int DelFullRow(int y)
{
    ...
    for(n=y;n>=0;n--)/*从当前行往上看*/
    {
        ...
        {
            if(!Table_DIBAN[n][m].var)totoal++;             /*没占有方格+1*/
            if(Table_DIBAN[n][m].var!=Table_DIBAN[n-1][m].var)  /*上行不等于下行就把上行传给下行 xor关系*/
            {
                Table_DIBAN[n][m].var=Table_DIBAN[n-1][m].var;
                Table_DIBAN[n][m].color=Table_DIBAN[n-1][m].color;
            }
        }
...
n  口口口口口口   8
 */
    for(n=t_DIBANy+3;n>=t_DIBANy;n--)
    {
        if(n<0 || n>=Vertical_boxs ){continue;} /*超过低线了*/
        for(m=0;m<Horizontal_boxs;m++)          /*水平的方向*/
        {
            if(!Table_DIBAN[n+full_numb][m].var)break;      /*发现有一个是空就跳过该行*/
        }
        if(m==Horizontal_boxs)                  /*找到满行了*/
        {
            if(n==t_DIBANy+3)                   /*第一次献给了n,最高的*/
    ...
    if(full_numb)
    {
        ...
        for(n=top;n<t_DIBANy+4;n++)
        {
            ...
            for(m=0;m<Horizontal_boxs;m++)  /*水平的方向*/
            {
                if(Table_DIBAN[n][m].var)
                    setfillstyle(SOLID_FILL,Table_DIBAN[n][m].color);/*Table_DIBAN[n][m].color*/
                else
                    setfillstyle(SOLID_FILL,BgColor);
        ...
        Showdefen(defen);
        if(speed!=defen/speed_step)
          {speed=defen/speed_step; ShowSpeed(speed);}
        else
          {ShowSpeed(speed);}
    }
}
...
int MkNextBox(int box_numb)
{
    ...
    t_DIBANx=(Curbox_x-Sys_x)/BSIZE;
    t_DIBANy=(Curbox_y-Sys_y)/BSIZE;
    for(n=0;n<4;n++)
    {
        for(m=0;m<4;m++)
        {
            if( ((FANGKUAIs[current_box_numb].box[n/2]) & mask) )
            {
                Table_DIBAN[t_DIBANy+n][t_DIBANx+m].var=1;/*这里设置游戏板*/
                Table_DIBAN[t_DIBANy+n][t_DIBANx+m].color=FANGKUAIs[current_box_numb].color;/*这里设置游戏板*/
            }
            mask=mask/(2);
            if(mask==0)mask=128;
        }
...
void EraseBox(int x,int y,int box_numb)
{
    ...
    for(n=0;n<4;n++)
    {
        ...
        {
            if( ((FANGKUAIs[box_numb].box[n/2]) & mask) )/*最左边有方块并且当前游戏板也有方块*/
            {
...
{
    int n,m,t_DIBANx,t_DIBANy;          /*t_DIBANx 当前方块最左边在游戏板的位置*/
    int mask;
    if(direction==MoveLeft)             /*如果向左移*/
    {
        mask=128;
        x-=BSIZE;
        t_DIBANx=(x-Sys_x)/BSIZE;
        ...
        for(n=0;n<4;n++)
        {
            for(m=0;m<4;m++)    /*看最左边四个单元*/
            {
                if((FANGKUAIs[box_numb].box[n/2]) & mask)/*最左边有方块并且当前游戏板也有方块*/
                {
                    ...
                    else if(Table_DIBAN[t_DIBANy+n][t_DIBANx+m].var) /*左移一个方块后，此4*4的区域与游戏板有冲突*/
                    {
                        return(false);
                    }
                }
    ...
    else if(direction==MoveRight)       /*如果向右移*/
    {
        ...
        for(n=0;n<4;n++)
        {
            ...
            {
                if((FANGKUAIs[box_numb].box[n/2]) & mask)/*最右边有方块并且当前游戏板也有方块*/
                {
                    if((x+BSIZE*m)>=(Sys_x+BSIZE*Horizontal_boxs) )return(false);/*碰到最右边了*/
                    else if( Table_DIBAN[t_DIBANy+n][t_DIBANx+m].var)
                    {
                        return(false);
                    }
                }
    ...
    else if(direction==MoveDown)        /*如果向下移*/
    {
        ...
        for(n=0;n<4;n++)
        {
            ...
            {
                if((FANGKUAIs[box_numb].box[n/2]) & mask)/*最下边有方块并且当前游戏板也有方块*/
                {
                    if((y+BSIZE*n)>=(Sys_y+BSIZE*Vertical_boxs) ||  Table_DIBAN[t_DIBANy+n][t_DIBANx+m].var)
                    {
                        flag_newbox=true;
                        break;
                    }
        ...
        for(i=0;i<8;i++)
        {
            if(i%4==0 && i!=0)              /*表示转到方块的下一行了*/
            {
                ...
                x=ls_x;
                       ...
                       if((FANGKUAIs[box_numb].box[ii])&mask)
            {
                bar(x,y,x+BSIZE,y+BSIZE);
                line(x,y,x+BSIZE,y);
                line(x,y,x,y+BSIZE);
...
void main()
{
    ...
    show_box(Curbox_x,Curbox_y,current_box_numb,FANGKUAIs[current_box_numb].color);
    show_box(LeftWin_x,Curbox_y+200,nextbox,FANGKUAIs[nextbox].color);
    ...
    while(1)
    {
        ...
        switch(key)
        {
            ...
            case VK_DOWN:
                ...
                break;
            case VK_UP:/*旋转方块*/
                if(MoveAble(Curbox_x,Curbox_y,FANGKUAIs[current_box_numb].next,MoveRoll))
                {EraseBox(Curbox_x,Curbox_y,current_box_numb);current_box_numb=FANGKUAIs[current_box_numb].next;
        ...
        {
            printf("game over,thank you! your defen is %d",defen);
            getch();
            break;
        }
    }
